#!/usr/bin/env python3
"""Embed data/ads-daily-report.json into ads-daily-report.html."""
...
JSON_PATH = ROOT / "data" / "ads-daily-report.json"
HTML_PATH = ROOT / "ads-daily-report.html"
...
def sanitize(data: dict) -> dict:
    ...
    if as_of:
        for key in ("yaha_daily", "yaha_region_daily"):
            for row in data.get(key, []):
                if "d0_retention" in row and row.get("date") and not d1_complete(row["date"], as_of):
                    row["d0_retention"] = None
    ...
    if isinstance(channels, list):
        data["yaahlan_android_channel_yesterday"] = [
            row for row in channels if row.get("ad_channel") in allowed
        ]
    return data
